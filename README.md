...
MovieBox video downloads use the authenticated visitor plus `play-info/v2` DASH
flow. The server fetches the signed MPD through its public-network guard and
uses FFmpeg stream copy/remuxing, so the transfer is low CPU and does not
transcode. FFprobe must find a video stream with a duration matching the
provider metadata before the file can become `ready`; the short upgrade clip is
therefore rejected. DASH output restarts from zero when it cannot be safely
resumed. Signed JWT and CloudFront authorization values are kept in memory or
request headers only and are never logged.

Optional [completion notifications](docs/server/notifications.md) push a "ready to watch" message to your phone the moment a download lands.

An optional [MCP endpoint](docs/server/mcp.md) lets an agent search the catalog and queue downloads for you. It is disabled until you configure a bearer token, and it reuses the same quality, storage, and safety rules as the browser UI.
