...
- Added `src/server/config.rs` with validated server configuration loading for bind address, SQLite URL, media/config/partial roots, admin/session secret files, max height, one-worker enforcement, 10 GiB reserve, Jellyfin base URL, and log format.
- Rejected unsafe roots such as `/` and `/mnt/mac-remote`, plus unsafe secret file permissions for mounted secrets.
...
- `cargo test --locked --features server --test server_auth` — passed, 7 tests.
